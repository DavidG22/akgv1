class IndividualContact < ActiveRecord::Base
  belongs_to :organization
   attr_accessible :first_name, :last_name, :position,
                   ...
                   :extension, :contact_image, :company_contact_id,:note,:file_description,:attachment, :city, :website,
          :state, :zip_code,:mobile_number, :street,
          :name, :work_phone, :facebook_url, :twitter_url, :linkedin_url, :messanger_type, :messanger_id, :time_zone, :subscribe_blog_mail, :subscribe_blog_date, :company_name, :is_csv, :description, :opportunity
#   accepts_nested_attributes_for :company_contact, :reject_if => lambda { |a| a[:company_name].blank? && a[:company_strength_id].blank? }, :allow_destroy => true
  ...
  attr_accessor :contact_id,:note,:file_description,:attachment,
