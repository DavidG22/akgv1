...
#  belongs_to :company_contact, :foreign_key=> "contact_ref_id"
  ...
  attr_accessible :is_current,:is_active,:contact,:amount, :attempts, :initiated_by, :is_public, 
          ...
          :stage_move_date, :tag_list, :hot_lead_token, :token_expiry_time, :next_priority_id, :assignee_id, :is_remote,:payment_status,:referrer,:visited,:location_by_api, :individual_contact_id, :user_label_id, :priority_type_id, :country_id
...
#  before_create :update_stage_move_date
  ...
  def self.to_csv
    CSV.generate do |csv|
      csv << ["Opportunity", "Stage", "Created On", "Created By", "Country", "Amount", "Next Action", "Contact Email", "Contact Name", "Priority"] ## Header values of CSV
      all.each do |lead|
        csv << [lead.title, 
                lead.deal_status_name,
                lead.created_at,
                lead.initiator_name,
                lead.current_country.present? ? lead.current_country.name : "NA",
                lead.amount,
                lead.latest_task_type_id.present? ? lead.last_task.name  : "No-Action",
                lead.contacts_email,
                lead.contacts_name,
                lead.priority_type.present? ? lead.priority_type.name : "" ] ##Row values of CSV
      end
    end
  end
...
#    params[:cre_by].present? || params[:cre_by_val].present? || params[:asg_by].present? || params[:asg_by_val].present? || params[:loc].present? || params[:loc_val].present? || params[:priority].present? || params[:priority_val].present?
    ...
    if params[:user_label] == true || params[:user_label] == "true"
      query += " and "if query.present?
      puts "-------------- params[:user_label_val]"
      p params[:user_label_val]
  ...
  def get_lead_with_priority
  {
    :id => id,
    :deal_name => title,
    :priority => self.priority_type.name 
  }
  end
