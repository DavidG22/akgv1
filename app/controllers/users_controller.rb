...
#*****************************************************************************************/
  ...
  before_filter :authenticate_admin, :except => [:update_profile_image,:save_tmp_img, :get_user_email, :profile,:save_profile_info, :update, :save_password,:load_header_count_user, :invite_user, :assign_unassign_deals, :assign_deal_to_user, :remove_lead, :edit_user, :block_unblock_user, :grant_revoke_admin, :save_daily_updates, :manage_daily_updates, :edit_daily_update, :delete_daily_update, :create_admin_user]
  ...
  def invite_user
   ...
   if !user.present?
    begin
      ...
      user.admin_type = 3
      #user.password=user.organization.name[0..5]+"@123" #commented for demo user
      ...
      respond_to do |format|
        ...
        if user.save
          user.invite!(@user) ## Commented for not sending email to create demo users
  ...
  def assign_unassign_deals
    ...
    if params[:assign_deal_ids].present?
      assign_deals = Deal.where("id IN (?)", params[:assign_deal_ids].split(","))
    ...
    if params[:unassign_deal_ids].present?
      unassign_deals = Deal.where("id IN (?)", params[:unassign_deal_ids].split(","))
...
def save_tmp_img
	...
	puts ">>>>>>>>>>>..this is before checking user"
      ...
      if !@user.image.present?
        image = Image.create(organization: @current_organization, image: params[:user][:profile_image], imagable: @user)
      end
  ...
  def update_profile_image
	...
	respond_to do |format|
	  @tmpimage=TempImage.find_by_id params[:id]
