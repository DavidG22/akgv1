...
#*****************************************************************************************/
  ...
  # before_filter :notify_reminder #Display flash message to notify reminder
  ...
  #     flash[:notice]="Reminder Notification will send at #{r.reminder_date.strftime("%m %s")}"
  #   end
  # end
...
#        end
  ...
  def get_country_states
    country = Country.find params[:id]
    states = ISO3166::Country[country.ccode].states.map { |k, v| v['name'] }
    respond_to do |format|
...
#    AttentionDealWorker.perform_async(current_user.id)
   ...
   if !session[:from_mail].nil? || !session[:from_mail].blank?  
    ...
    if session[:return_to].present? && session[:return_to].include?("google_oauth2")
      "/"
    else
