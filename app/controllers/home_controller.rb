...
#*****************************************************************************************/
  ...
  skip_before_filter  :authenticate_user!, :only => [:index, :pricing, :notfound, :terms, :privacy, :security,:contact_us, :clear_cache,:api_contact_us, :download_user, :report_a_bug, :show_bug_report, :post_comment, :send_feedback]
  before_filter :authenticate_admin, :only => [:activities]
  ...
  def dashboard
    begin
      ...
      @upcoming_tasks=@tasks.where("is_completed=? AND DATE_FORMAT(DATE_ADD(due_date, INTERVAL #{Time.zone.now.utc_offset} second), '%Y/%m/%d') > ?", false, Time.zone.now.strftime("%Y/%m/%d")).limit(5)
      ...
      @total_leads = @current_organization.deals.count
      
      @completed_tasks = Task.by_is_completed.count
  ...
  def summary
    ...
    if @last_deal.present?
      ...
      if DealMove.by_deal_id_and_status_won(@last_deal.id, @last_deal.deal_status_id).first.present?
...
group by created_by order by count(*) desc")
 ...
 def usage
  ...
  def daily_updates
    @deals = @current_organization.deals
  end
 ...
 def download_user
  ...
  def send_feedback
    ...
    Notification.send_feedback_to_user(name,email).deliver
