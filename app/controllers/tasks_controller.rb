...
#*****************************************************************************************/
  ...
  before_filter :retrieve_tasks, :only => [:complete, :start_task, :all_task, :today_task, :overdue_task, :upcoming_task, :calendar_data, :task_filter, :fetch_tasks]
...
#          task.created_by = current_user.id
    ...
    #          flash[:notice]="Task has been saved successfully."
              ...
              task_info={status: true, msg: "Task has been saved successfully.", ttype: @task.task_type.present? ? @task.task_type.name : "", date: @task.due_date.strftime("%a %d %b %Y @ %H:%M"), tdate: task_tab(@task)}
...
#    end
  ...
  def reschedule_task
    task = Task.find_by_id params[:event][:id]
    if task.present? && params[:event][:due_date].present?
      task.update_column(:due_date, params[:event][:due_date].to_datetime)
    end
    render :json => "success"
  end
  ...
  def calendar_data
    if request.xhr?
      ...
      task.due_date= task.created_at unless task.due_date.present?
			...
			events << {:id => task.id, :img=>imgurl, :is_complete=> task.is_completed, :tasktype=> task.task_type.present? ? task.task_type.name : "" , :assign_to=> "#{(task.user.present? ? task.user.full_name : 'NA')}", :url=> "http://#{request.host_with_port}/leads/#{deal.id}", :title => "\n"+task.title+ "\n" + "Deal:" +" "+deal.title+ "\n" ,   :color=> TaskType::TASK_COLORS[task.task_type.name], :className=>"test", :description => "At - #{task.due_date.strftime('%H:%M')}\nTask - "+task.title+"\n Deal - " + deal.title+"\nAssigned To - "+(task.user ? task.user.full_name : ""), :start => "#{task.due_date.iso8601}", :end => "#{task.due_date.iso8601}", :allDay => false}
	    end
      end
    end
    respond_to do |format|
