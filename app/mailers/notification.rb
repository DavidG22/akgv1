class Notification < ActionMailer::Base
  ...
  def send_email(to,cc,bcc,subject,msg,file_name,file,activity_id, name) 
    @message  = msg
...
#    mail(to: email, subject: "WakeUpSales: New task assigned")
 ...
 def mail_to_admin_api(admin_email, source, link,contact_name, contact_email,contact_phone,initial_note,subject)
  ...
  def send_reminder_email(reminder)
    @reminder = reminder
    mail(to: @reminder.email_ids, subject: "WakeUpSales: Reminder Email")
  end

  def send_assign_unassign_mail(user,type,leads)
    @user = user
    @leads = leads
    @type = type
    mail(to: @user.email, subject: "WakeUpSales: #{@type=='assign' ? 'Assign' : 'Remove'} Leads Email")
  end

  def send_daily_updates(user, deal)
    @deal = deal
    @user = user
    @emails = @user.map{|u| u.email}.map(&:inspect).join(",")
  ...
  def send_feedback_to_support(name,email,company,desc)
    ...
    @company = company
    @desc = desc
    mail(to: "support@wakeupsales.com", subject: "WakeUpSales: Feedback from user (In app)")
